...
func (r *repository) NextID() application.ProductID {
	return application.ProductID(uuid.New())
}

func (r *repository) FindByID(id application.ProductID) (*application.Product, error) {
	var raw rawProduct
	query := `SELECT id, title, sku, price, available_count, image_url, image_width, image_height, color, material 
...
func (r *repository) Find(pageSpec *application.Page, filters *application.Filters) ([]*application.Product, error) {
	var items []*application.Product
	query := "SELECT id, title, sku, price, available_count, image_url, image_width, image_height, color, material FROM products"
...
func (r *repository) Add(item application.Product) error {
	_, err := r.connPool.Exec(
		`INSERT INTO products (id, title, sku, price, available_count, image_url, image_width, image_height, color, material) 
...
func mapToProduct(raw rawProduct) (*application.Product, error) {
	itemID, _ := uuid.FromBytes([]byte(raw.ID))
...
func applyPageSpec(query *string, pageSpec *application.Page) {
	if pageSpec == nil {
		return
	}
