...
func MakeHandler(pathPrefix string, endpoints Endpoints, errorLogger log.Logger, metrics *MetricsHolder) http.Handler {
	...
	s.Handle("/products", InstrumentingMiddleware(listProductsHandler, metrics, "ListProducts")).Methods(http.MethodGet)
	s.Handle("/products", InstrumentingMiddleware(createProductHandler, metrics, "CreateProduct")).Methods(http.MethodPost)
	s.Handle("/products/{id}", InstrumentingMiddleware(getProductByIDHandler, metrics, "GetProductByID")).Methods(http.MethodGet)
	return r
}
